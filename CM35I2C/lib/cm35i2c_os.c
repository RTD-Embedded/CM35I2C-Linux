/**
	...
	$Id: cm35i2c_os.c 150826 2025-11-17 19:05:33Z bkorpacz $
*/
...
int CM35I2C_Dma_Initialize(struct CM35I2C_Board_Descriptor *handle,
				...
				uint32_t buffer_size)
{
	 ...
	 * Disable interrupts
	 */
	result = CM35I2C_Dma_Configure_Interrupts(handle,
						func_block,
						channel,
						0,
						0);

	CM35I2C_Check_Result(result, "Error disabling DMA interrupts.");


	result = CM35I2C_Dma_Clear(handle,
					func_block,
					channel);

	CM35I2C_Check_Result(result, "Error clearing DMA.");


	result = CM35I2C_Dma_Clear_Interrupt(handle,
						func_block,
						channel,
						1,
						1,
	...
	CM35I2C_Check_Result(result, "Error clearing DMA interrupts.");
	...
	for (buff = 0; buff < num_buffers; buff++) {
		 ...
		 * Allocate buffer
		 */
		ioctl_request.dma.pci.region = CM35I2C_PCI_REGION_FB;
		ioctl_request.dma.pci.size = CM35I2C_PCI_REGION_ACCESS_32;
		ioctl_request.dma.function = CM35I2C_DMA_INITIALIZE;
		...
		ioctl_request.dma.pci.offset = func_block->dma_channel[channel].buffer_start_offset[buff] +
						CM35I2C_OFFSET_DMA_BUFFER_ADDRESS;
		ioctl_request.dma.buffer = buff;
        	result = CM35I2C_Dma(handle, &ioctl_request);
		CM35I2C_Check_Result(result, "Error initializing DMA via ioctl");

		result = CM35I2C_Dma_Buffer_Set_Size(handle,
							func_block,
							channel,
							buff,
							(buffer_size & CM35I2C_BIT_MASK_DMA_BUFFER_SIZE));

		CM35I2C_Check_Result(result, "Error setting DMA Buffer size");


		/**
		 * Clear the status
		 */
		result = CM35I2C_Dma_Reset_Buffer(handle,
							func_block,
							channel,
							buff);

		CM35I2C_Check_Result(result, "Error resetting DMA buffer.");

		/**
		 * Clear the buffer control
		 */
		result = CM35I2C_Dma_Buffer_Setup(handle,
							func_block,
							channel,
							buff,
							CM35I2C_DMA_BUFFER_CTRL_CLEAR);

		CM35I2C_Check_Result(result, "Error clearing buffer control register.");
...
int
CM35I2C_Dma_Read(struct CM35I2C_Board_Descriptor *handle,
				const struct CM35I2C_Function_Block *func_block,
				unsigned int channel,
				...
				void *local_buffer_ptr)
{
	...
	ioctl_request.dma.function = CM35I2C_DMA_READ;
	ioctl_request.dma.channel = channel;
	ioctl_request.dma.fb_num = func_block->fb_num;
	ioctl_request.dma.buffer_size = buffer_size;
	...
	return CM35I2C_Dma(handle, &ioctl_request);
...
int
CM35I2C_Dma_Write(struct CM35I2C_Board_Descriptor *handle,
				const struct CM35I2C_Function_Block *func_block,
				unsigned int channel,
				...
				void *local_buffer_ptr)
{
	...
	ioctl_request.dma.function = CM35I2C_DMA_WRITE;
	ioctl_request.dma.channel = channel;
	ioctl_request.dma.fb_num = func_block->fb_num;
	ioctl_request.dma.buffer_size = buffer_size;
	ioctl_request.dma.buffer_ptr = local_buffer_ptr;
...
int CM35I2C_General_RemoveISR(struct CM35I2C_Board_Descriptor *handle)
{
	...
	ioctl(handle->file_descriptor, CM35I2C_IOCTL_WAKEUP);

	return pthread_join(handle->pid, NULL);
}


void *CM35I2C_General_WaitForInterrupt(void *ptr)
{
	...
	handle = (struct CM35I2C_Board_Descriptor *) ptr;
	...
	while (1) {
		 ...
		 * activity.  Only the CM35I2C device file descriptor is of interest.
		 */
		 ...
		 * activity.  Only the CM35I2C file descriptor is of interest.
		 */
		...
		status = ioctl(handle->file_descriptor, CM35I2C_IOCTL_INTERRUPT_GET,
			  &ioctl_arg);
...
int
CM35I2C_General_InstallISR(struct CM35I2C_Board_Descriptor *handle, void (*isr_fnct))
{
	/*
	...
	if (pthread_create
		(&(handle->pid), NULL, CM35I2C_General_WaitForInterrupt,
		 handle) != 0) {
		errno = EFAULT;
		return -1;
	}
...
int 
CM35I2C_General_SetISRPriority(struct CM35I2C_Board_Descriptor *handle,
				   int priority)
{
