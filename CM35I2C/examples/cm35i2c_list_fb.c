...
int main(int argument_count, char **arguments)
{
	...
	int result;
	int fb_num = 0;
	struct CM35I2C_Board_Descriptor *board;
	...
	result = CM35I2C_Board_Open(minor, &board);
	...
	if(reset_option_given == OPTION_GIVEN) {
		...
		result = CM35I2C_Gbc_Board_Reset(board);
		CM35I2C_Check_Result(result, "Could not reset board");
		printf("success.\n\n");
	}

	result = CM35I2C_Gbc_Get_Revision(board, &rev_num);
	CM35I2C_Check_Result(result, "Error getting board revision number");

	result = CM35I2C_Gbc_Get_Fpga_Build(board,
						&fpga_num);
	CM35I2C_Check_Result(result, "Error getting FPGA build number.");

	result = CM35I2C_Gbc_Get_Pdp_Number(board,
						&pdp_num);
	CM35I2C_Check_Result(result, "Error getting PDP number.");
	...
	fb_num = 0;
	while ((fb_num < CM35I2C_MAX_FB) && (errno != ERANGE)) {
		result = CM35I2C_Function_Block_Open(board,
		...
		if (errno != ERANGE) {
			...
			sprintf(errMsg, "Could not open function block %d.",
				fb_num);
			CM35I2C_Check_Result(result, errMsg);

			switch (my_func_block.type) {

			case CM35I2C_FUNC_BLOCK_ADC:
				printf
			...
			case CM35I2C_FUNC_BLOCK_DIO:
				printf
				    ("  FB%d: DIO (Type 0x%x Subtype %d), with %d DMA Channels (%d buffers each)\n",
				     ...
				     my_func_block.num_dma_buffers);
				break;
			case CM35I2C_FUNC_BLOCK_SYNCBUS:
				printf
				    ("  FB%d: Syncbus Module (Type 0x%x Subtype %d), with %d DMA Channels (%d buffers each)\n",
				     ...
				     my_func_block.num_dma_buffers);
				break;
			case CM35I2C_FUNC_BLOCK_FLASH_PROGRAMMER:
				printf
				    ("  FB%d: RTD Firmware Block (Type 0x%x Subtype %d), with %d DMA Channels (%d buffers each)\n",
				     my_func_block.fb_num,
				     my_func_block.type,
				     my_func_block.sub_type,
				     my_func_block.num_dma_channels,
				     my_func_block.num_dma_buffers);
				break;
			case CM35I2C_FUNC_BLOCK_I2C3300:
				printf("  FB%d: I2C Block (Type 0x%x Subtype %d), with %d DMA Channels (%d buffers each)\n",
				     my_func_block.fb_num,
				     my_func_block.type,
				     my_func_block.sub_type,
				     my_func_block.num_dma_channels,
				     my_func_block.num_dma_buffers);
				break;
			case CM35I2C_FUNC_BLOCK_INVALID:
				// Do nothing
				break;
			default:
	...
	printf("\nClosing Board\n");
	result = CM35I2C_Board_Close(board);
